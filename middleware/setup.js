...
export default async function ({store, redirect}) {
    if (store.$auth.loggedIn) {
        ...
        let {data} = await axios.get('http://localhost:8000/api/settings/', {headers:{Authorization: store.$auth.$storage._state['_token.local']}})
        ...
        if (!missingSetupSettings) {
            ...
            //  - reduce: get only the settings keys
            //  - filter: remove settings that are not required
            let keys = data.reduce(setting => setting.key)
        ...
        data = await axios.get('http://localhost:8000/api/subjects/', {headers:{Authorization: store.$auth.$storage._state['_token.local']}})
        let noSubjects = data.length < 1

        // Check if the user has at least ONE event in his schedule
        data = await axios.get('http://localhost:8000/api/events/', {headers:{Authorization: store.$auth.$storage._state['_token.local']}})
