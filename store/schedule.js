...
export const getters = {
  ...
  setting: (state, getters, rootState, rootGetters) => (settingName) => {
    ...
    settings = settings.filter(setting => setting.name == settingName)
    if (!settings.length) {
  ...
  weekType: (state, getters) => date => {
    ...
    return moment(date).isoWeek() % 2 === 0 && startingWeekIsEven ? base : other
  },
  ...
  coursesIn: (state, getters) => (start, upto, debug=null) => {
    ...
    for (let current = start; current.isBefore(upto); current.add(1, 'day')) {
      ...
      if(debug) console.log(`${current.format('D/M/Y')}#${weekday}:${weekType}`)
      let dayBaseEvents = baseEvents.filter(ev => {
        return ev.day === weekday && (ev.week_type === 'both' || ev.week_type === weekType)
      });
      if(debug) console.log(dayBaseEvents.map(e => `${e.subject.abbreviation.toUpperCase()}@${e.start}`))
      ...
      for (const baseEvent of dayBaseEvents) {
        ...
        if (deletions.length) {
          deleted = deletions.filter(deletion => deletion.event.id === baseEvent.id)
                             .length >= 1;
        } else {
          deleted = false;
        }
      ...
      if (additions.length) {
        ...
        for (const addedEvent of additions.filter(
          event => pdate(event.current).isSame(current)
        )) {
          computedEvents.push({
            ...addedEvent,
            day: weekday,
  ...
  nextCourseOf: (state, getters) => (subjectSlug) => {
    ...
    courses = courses.sort((a, b) => pdate(a.date).isAfter(pdate(b.date)) ? 1 : -1)
    return courses[0]
  },
  ...
  offdays: (state, getters) => {
    let offdays
    let offdaySeries = getters.setting('schedule/offdays') // get offdays
                              .split('\n') // split by line
                              .forEach(offday => offday.replace('\r', '')) // fuck windows
    for (const offdaySerie of offdaySeries) { // for each line
      if (offdaySerie.match(/.+\s*-\s*.+/)) { // if its a range (start date - end date)
