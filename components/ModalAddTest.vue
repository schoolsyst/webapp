...
export default {
  ...
  methods: {
    pickSubject($event) {
      ...
      document
        .querySelectorAll("#modal_add-test select option")
        .forEach((e) => {
    ...
    async addTest() {
      ...
      if (!this.mutMax) {
        errs.push(
          `Choisissez l'unité de cette note (vous pouvez changer cela plus tard)`
        )
      }
      if (!this.mutWeight) {
        errs.push(
          `Choisissez le coefficient de la note (vous pouvez changer cela plus tard)`
        )
      }
      if (!this.mutDate) {
        ...
        if (!parsedDate.isValid()) {
          ...
          errs.push(
            "La date du contrôle doit être dans le futur, Marty! Tu veux créer un paradoxe temporel‽"
          )
        }
      }
